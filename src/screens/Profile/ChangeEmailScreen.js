...
const ChangeEmailScreen = ({ navigation }) => {
  ...
  const handleRequestEmailChange = async () => {
    ...
    try {
      ...
      // Step 1: Validate email and check if exists
      const checkResult = await profileService.requestEmailChange(currentUser.id, form.newEmail);
      ...
      const otpResult = await sendOTP(form.newEmail);
      if (!otpResult.success) {
        Alert.alert('Lỗi', otpResult.message);
        return;
      }

      Alert.alert('Thành công', `OTP đã được gửi đến ${form.newEmail}\n`);
      setStep('otp');
    } catch (error) {
  ...
  const handleConfirmEmailChange = async () => {
    ...
    try {
      ...
      const verifyResult = await verifyOTP(form.newEmail, form.otp, 'signup');
      ...
      const updateResult = await profileService.confirmEmailChange(
        currentUser.id,
        form.newEmail,
        form.otp
      );
  ...
  const handleResendOTP = async () => {
    try {
      setLoading(true);
      const result = await sendOTP(form.newEmail);
      if (result.success) {
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.section}>
        <Text style={styles.label}>Email mới: {form.newEmail}</Text>
      </View>

      <View style={styles.section}>
        <Text style={styles.label}>Nhập OTP</Text>
        <Text style={styles.hint}>Vui lòng nhập mã OTP được gửi đến email mới</Text>
        <TextInput
          style={styles.input}
