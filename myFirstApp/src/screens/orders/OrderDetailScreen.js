...
export default function OrderDetailScreen({ route, navigation }) {
  ...
  const { orders, selectedOrder, getOrderDetail, cancelOrder, advanceOrderStatus } = useOrderStore();
  ...
  useEffect(() => {
    if (!order) {
      ...
      getOrderDetail(orderId).finally(() => setLoadingDetail(false));
    }
  }, [orderId]);

  // Use selectedOrder if main order not found
  if (!order) order = selectedOrder;

  if (loadingDetail || !order) {
    return (
      <View style={styles.container}><Text style={styles.notFound}>Đang tải...</Text></View>
    );
  }
  ...
  const getStatusColor = (status) => ORDER_STATUS_COLORS[status] || '#9E9E9E';
  const getStatusLabel = (status) => ORDER_STATUS_LABELS[status] || status;
  const getStatusIcon = (status) => STATUS_ICONS[status] || 'help-circle';
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.headerCard}>
        ...
        <Chip
          ...
          textStyle={{ color: getStatusColor(order.status), fontWeight: 'bold' }}
          icon={() => <MaterialCommunityIcons name={getStatusIcon(order.status)} size={16} color={getStatusColor(order.status)} />}
        >
